...
pub fn create_instance(commands: &mut Commands) -> Entity {
    let camera_entity = Some(commands.spawn_bundle(PerspectiveCameraBundle::default()).id());
    ...
    let player_entity = commands
        .spawn_bundle((
            ...
            ColliderBuilder::capsule_y(0.25, 0.25)
                ...
                .collision_groups(InteractionGroups::all().with_memberships(interaction_flags::PLAYER.into()))
        )) //.id();//.unwrap();
        .with_children(|parent| {
            // child cube
            parent.spawn_bundle(PerspectiveCameraBundle::default());
        }).id();

    // Append camera to player as child.
    // commands.entity(player_entity).push_children(&[camera_entity.unwrap()]);
...
pub fn update_player(
    ...
    for (mut player, transform, rigid_handle) in &mut queries.q0_mut().iter_mut() {
        ...
        let fwd = transform.local_x().normalize();
        let right = Vec3::cross(fwd, Vec3::Y).normalize();
